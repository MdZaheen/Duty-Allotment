  ...
  try {
    ...
    for (const schedule of schedules) {
      ...
      for (const room of rooms) {
        ...
        // Sort eligible professors by duty count (to distribute duties evenly)
        eligibleProfessors.sort((a, b) => {
          ...
          // If duty counts are equal, sort by designation (higher seniority first)
          if (countA === countB) {
            ...
            const rankA = designationOrder[a.designation as keyof typeof designationOrder] || 0;
            const rankB = designationOrder[b.designation as keyof typeof designationOrder] || 0;
            
            return rankB - rankA; // Higher rank first
          }
          
          return countA - countB; // Lower duty count first
        });
        
        // Get the professor with the least duties
        const selectedProfessor = eligibleProfessors[0];
